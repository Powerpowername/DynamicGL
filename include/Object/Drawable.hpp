...
protected:
    ...
    glm::vec4 _clip_coord; // 裁剪坐标

    bool _has_shadow_buffer; // 是否启用阴影
    bool _enabled;

    GLuint _shadow_tex_id; // 阴影纹理id

    GLuint _shadow_mapping_effect;        // 阴影映射效果
    GLuint _window_width;                 // 窗口宽度
    GLuint _window_height;                // 窗口高度
    GLuint _shadow_buffer_texture_width;  // 阴影纹理宽度
    GLuint _shadow_buffer_texture_height; // 阴影纹理高度
public:
    Drawable()
    {
